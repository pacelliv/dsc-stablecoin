...
export const burn = async (burnAmount: string) => {
  ...
  try {
    if (parseFloat(positionData?.debt!) < parseFloat(burnAmount)) {
      throw new Error("Error: Insufficient DSC balance.");
    }

    await (await dsc.approve(await dscEngine.getAddress(), ethers.parseEther(burnAmount))).wait(1);

    const burnTxResponse = (await dscEngine.burn(ethers.parseEther(burnAmount))) as TransactionResponse;

    (document.getElementById("transaction-form") as HTMLFormElement).reset();
    safeRenderTransactionStatusPanel(
      {primaryAction: "burn", primaryAmount: burnAmount, primaryAmountUnit: "DSC"} as Actions,
      burnTxResponse.hash,
    );
