...
export const deposit = async (depositAmount: string) => {
  ...
  try {
    const txResponse = (await dscEngine.deposit({
    ...
    safeRenderTransactionStatusPanel(
      {primaryAction: "deposit", primaryAmount: depositAmount, primaryAmountUnit: "ETH"} as Actions,
      txResponse.hash,
    );
