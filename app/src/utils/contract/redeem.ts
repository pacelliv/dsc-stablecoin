...
export const redeem = async (redeemAmount: string) => {
  ...
  try {
    const txResponse = (await dscEngine.redeem(ethers.parseEther(redeemAmount))) as TransactionResponse;
    (document.getElementById("transaction-form") as HTMLFormElement).reset();
    safeRenderTransactionStatusPanel(
      {primaryAction: "withdraw", primaryAmount: redeemAmount, primaryAmountUnit: "ETH"} as Actions,
      txResponse.hash,
    );
